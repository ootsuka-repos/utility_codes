...
def main():
    ...
    # インスタンス作成
    client = ComfyUIClient(CONFIG["server_address"])
    processor = ImageProcessor(CONFIG, THEMES)
    
    while True:
        try:
            # 入力動画の選択
            image_path = processor.select_random_image(current_theme)
            if not image_path:
                print("入力動画が見つかりません。終了します。")
                break
                
            # プロンプトの準備
            prompt = processor.prepare_prompt(image_path, current_theme)
            ...
            # 動画保存
            output_path = processor.save_output_image(history)
            if output_path:
